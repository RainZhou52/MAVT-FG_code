...
class Select(nn.Module):
    def __init__(self, bs, device, embed_dim, num_patches, drop_ratio):
    ...
    def forward(self, cls_video, cls_audio):
        cls_video_n = cls_video.reshape(cls_video.shape[0], 768)
        cls_audio_n = cls_audio.reshape(cls_video.shape[0], 768)
        ...
        cls_audio_n = cls_audio_n.flatten()

        fusion_cls = torch.cat((self.cls_video_w * cls_video_n, self.cls_audio_w * cls_audio_n), -1)
        FF = nn.Linear(self.embed_dim * cls_video.shape[0] * 2, self.num_patches).to(self.device)

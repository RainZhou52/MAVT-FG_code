...
from AVF_Decoder import AVF_Decoder, PatchEmbed, Encoder_layer

class MAVT_FG(nn.Module):
    def __init__(self, args, img_size=224, patch_size=16, in_c=3, num_classes=1000,
        ...
        for _ in range(depth - 1):
            layer = AVF_Decoder(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                                qk_scale=qk_scale,
        ...
        self.Select = Select(bs=args.batch_size, device=args.device, embed_dim=embed_dim,
                             num_patches=num_patches, drop_ratio=drop_ratio)
    ...
    def forward_features(self, video, audio):
        ...
        for _ in range(self.depth - 1):
            layer = AVF_Decoder(dim=self.embed_dim, num_heads=num_heads, mlp_ratio=self.mlp_ratio,
...
def MAVT_FG_mm(args, num_classes: int = 21843, has_logits: bool = True):
    model = MAVT_FG(args=args,
                    img_size=224,
                    patch_size=16,
                    embed_dim=768,
                    depth=6,
